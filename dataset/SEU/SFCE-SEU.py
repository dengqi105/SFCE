...
sensor_index = [0, 1, 2, 3, 4, 5, 6, 7]
K = 7
L = 512  # sampele length
kernel_size = 45  # 45 is OK
nums = 200  # sample number for each class
step = int(L / 2)
print("K:", K)
print("L:", L)
...
data_path = r"./dataset/{}/".format(dataset_name)
...
for i in label:
    ...
    for s in sensor_index:
        d = pd.to_numeric(data.iloc[16:nums * step + L, s]).values
        datas.append(d)
    datas = np.row_stack(datas).T
    samples = []
    for j in range(nums):
        sample = datas[j * step:j * step + L, :].transpose()
